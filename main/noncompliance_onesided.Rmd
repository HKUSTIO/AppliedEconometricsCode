...
In this section, we implement the estimation of the intent-to-treat effect and the local average treatment effect in the case of one-sided compliance in the chapter "Non-compliance". The functions called from `AppliedEconometrics` are defined in `R/functions_noncompliance.R`. First, we generate the latent outcomes of the sample, and then generate the treatment vector and the realized outcomes. When doing so, we consider a situation where the average treatment effect differs for each compliance type. Based on the realized treatment assignment vector, the realized treatment receipt vector, and the realized outcomes, we estimate the treatment receipt and the treatment assignment effect. Next, using these estimated values, we estimate the local average treatment effect.
...
data_realized_excluded <-
  AppliedEconometrics::generate_data_noncompliance_oneside(
