...
Here, we implement the estimation method for sharp regression discontinuity design using a discrete score with the `RDHonest` package. The functions called from `AppliedEconometrics` are defined in `R/functions_rdrobust.R` and `R/functions_rdhonest.R`.
...
### Cluster-robust Variance Estimation: Smaller Example
...
While the correction by the `masspoints` option in `rdrobust` is effective, it is advisable to consider this correction as applicable only to cases where random duplicate observations occur in continuous scores.
