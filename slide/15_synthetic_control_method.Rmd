...
$$
\sum_{i = 2}^{N + 1} w_i^\ast Y_{it}^\ast(0) = Y_{1t}^\ast(0), t = 1, \cdots, T_0,
$$
and
$$
...
$$
\begin{split}
&Y_{1t}^\ast(0) - \sum_{i = 2}^{N + 1} w_i^\ast Y_{it}\\
&=\theta_t\left(Z_1 - \sum_{i = 2}^{N + 1} w_i^\ast Z_i\right)\\
&+\lambda_t \left(\mu_1 - \sum_{i = 2}^{N + 1} w_i^\ast \mu_i \right)\\
...
&+ \boldsymbol{\lambda} \left(\mu_1 - \sum_{i = 2}^{N + 1} w_i^\ast \mu_i \right)\\
...
$$
\begin{split}
\left(\mu_1 - \sum_{i = 2}^{N + 1} w_i^\ast \mu_i \right) & = (\boldsymbol{\lambda}' \boldsymbol{\lambda})^{-1} \boldsymbol{\lambda}' \left(\mathbf{Y}_1 - \sum_{i = 2}^{N + 1} w_i^\ast \mathbf{Y}_i\right)\\
...
- The first term is mean zero because $\boldsymbol{\epsilon}_1$ has mean zero conditional on $\boldsymbol{\lambda}$.
- The third term is mean zero because $w_i^\ast$ contains information on $t \le T_0$ and is independent of errors at $t > T_0$.
- The second term may not be mean zero: bias term because $w_i^\ast$ and $\boldsymbol{\epsilon}_i$ are correlated.
...
## DID, SCM, Synthetic DID

- If the control period is short, only DID works
- If the control period is long, all methods can work
   - If the treated unit is single, only SCM works
   - If the treated unit is multiple, DID and Synthetic DID can work
       - DID requires a relatively larger cross-sectional sample
