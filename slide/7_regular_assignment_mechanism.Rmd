...
- Suppose not. Then, there is a balancing score $b(w)$ and values $x, x'$ such that $b(w) = b(w')$ but $e(w) \neq e(w')$.
- Then, we have:
$$
...
where $\tau_{sp}$ is:
$$
\tau_{sp}(W_i) \equiv \mu_1(w) - \mu_0(x) = \mathbb{E}[Y_i^\ast(1) - Y_i^\ast(0) | W_i = w].
$$
...
- To avoid extreme valeus in the propensity score, one may stratify samples according to the value of the propensity score: For $j = 0, 1, \cdots, J$, $b_0 = 0 \le \cdots \le b_j \le \cdots \le b_J = 1$, let $B_i(j)$ be a binary indicator of $b_{j - 1} < \hat{e}(W_i) \le b_j$.
...
$$
and $W_i^p$ is considered to be the proxiy to the potential outcome, such as the lagged outcomes.
