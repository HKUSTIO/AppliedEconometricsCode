...
- However, there are many cases in which a subject assigned to treatment does not receive the treatment.
  - Supplying supplements to a village to reduce infant mortality, but some of the villagers could not receive the supplements.
...
$$
\pi_{co} \equiv \frac{N_{co}}{N}, \pi_{nc} \equiv \frac{N_{nc}}{N} = 1 - \pi_{co}.
$$
...
$$
\widehat{\mathbb{V}}(\widehat{ITT}_D) \equiv \frac{s_{D, 0}^2}{N_0} + \frac{s_{D, 1}^2}{N_1}.
$$
...
$$
ITT_{Y, co} \equiv \frac{1}{N_{co}} \sum_{i: G_i = co}\{Y_i^\ast[1, D_i^\ast(1)] - Y_i^\ast[0, D_i^\ast(0)]\}, 
$$
and:
$$
ITT_{Y, nc} \equiv \frac{1}{N_{nc}} \sum_{i: G_i = nc}\{Y_i^\ast[1, D_i^\ast(1)] - Y_i^\ast[0, D_i^\ast(0)]\},
$$
...
- We cannot identify $ITT_{Y, co}$ and $ITT_{Y, nc}$ because we do not know the compliance status of each subject and hence we cannot condition on compliance status.
- $ITT_{Y, nc}$ is not informative, because they never receive the treatment, irrespective of the assignment.
...
$$
ITT_{Y, nc} \equiv \frac{1}{N_{nc}} \sum_{i: G_i = nc}\{Y_i^\ast[1, D_i^\ast(1)] - Y_i^\ast[0, D_i^\ast(0)]\} = 0.
$$
- Therefore:
$$
...
$$
ITT_{Y, co} = \frac{1}{N_{co}} \sum_{i: G_i = co}\{Y_i^*(1) - Y_i^*(0)\},
$$
...
- Because we have $\widehat{ITT}_Y$ and $\widehat{ITT}_D$, we can estimate $\tau_{late}$ by:
$$
...
$$
- Therefore, the instrumental variable estimator for $\tau_{late}$, which is $\hat{\gamma}_{ols}/\hat{\pi}_{co}$, is numerically the same as $\widehat{ITT}_{Y, co}$.

# Two-sided noncompliers

## Generalize compliance behaviors

- In experiments, we may be able to ensure that subjects who are not assigned treatment do not receive treatment.
...
- We consider 4 compliance status: Always takers, never takers, compliers, and defiers.
$$
G_i =
\begin{cases}
...
- There are two underlying compliance status for each pair of $Z_i$ and $D_i$:<footnote> Table 24.3 of Imbens and Rubin (2015) </footnote>
...
- The ITT analysis is unchanged with the two-sided noncompliers.
...
- Formally:
- __Assumption: Monotonicity__
  - For all subjects $D_i^\ast(1) \ge D_i^\ast(0)$.


## Identification of $ITT_{Y, co}$

- Under the exclusion restriction for never takers, always takers, and the monotonicity, we have:
$$
