...
- When $\rho_k$ and $\rho_u$ have the same sign, the downward bias of the homoskedastic standard error is greater when:
    - The within-cluster correlation of the regressor is higher;
    - The within-cluster correlation of the error is higher;
    - The average number of units per cluster is larger.
...
- Use the empirical distribution of $\{\hat{\beta}^b\}_{b = 1}^B$ to conduct inference.
...
- Repeat the following steps for $b = 1, \cdots, B$:
    ...
    1. Calculate the Wald test statistic $w^b = (\hat{\beta}^b - \hat{\beta})/s_{\hat{\beta}^b}$.

## Wild cluster bootstrap

- Reject the null hypothesis $H_0$ if the Wald statistic $w = |\hat{\beta} - \beta_0|/s_{\hat{\beta}}$ is greater than $(1 -\alpha/2)$-percentile of $\{w^b\}_{b = 1}^B$.
- This procedure is specific to a single null hypothesis.
