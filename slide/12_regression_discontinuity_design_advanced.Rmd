...
## Treatment assignment and receipt
...
- The sharp RD estimator of the effect of treatment assignment $Z_i$ on the outcome $Y_i$ now estimates:
$$
\begin{split}
...
\end{split}
$$
- We call this _first-stage effect_ and denote by $\tau_{1st}$.
- $\tau_{itt}$ and $\tau_{1st}$ are both sharp RD parameters and inherit the properties we have discussed so far.
...
&= \frac{\tau_{itt}}{\tau_{1st}}.
\end{split}
$$

## Biac-correction and robust standard error

- We first estimate $\tau_{itt}$ and $\tau_{1st}$ with a band width $h$ to obtain $\hat{\tau}_{itt}(h)$ and $\hat{\tau}_{1st}(h)$.
- Then, we can estimate $\tau_{frd}$ by $\hat{\tau}_{frd}(h) \equiv \hat{\tau}_{itt}(h) / \hat{\tau}_{1st}(h)$.
...
$$
\hat{\tau}_{frd}(h) - \tau_{frd} = \frac{\hat{\tau}_{itt}(h) - \tau_{itt}}{\tau_{1st}} - \tau_{itt} \cdot \frac{\hat{\tau}_{1st}(h) - \tau_{1st}}{\tau_{1st}^2} + R.
$$
