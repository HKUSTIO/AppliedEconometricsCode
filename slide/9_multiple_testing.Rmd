...
- __Definition__: _Family-wise error rate_: The probability of rejecting at least one null hypotheses that are true:
$$
...
- Depends on the following inequality:
$$
\begin{split}
...
- The Bonferroni correction: Reject $H_0^m$ if the piece-wise p-value for $H_0^m$ is $p^m \le \alpha/M$. Then, $\text{FWER} \le \alpha M_0/M \le \alpha$.

## Bonferroni correction

- Equivalently, we can adjust the p values as
$$
\tilde{p}^m = M p^m, m = 1, \cdots, M,
$$
and reject $H_0^m$ if $\tilde{p}^m \le \alpha$.

## Why Bonferroni Correction is Conservative?

- Bonferroni correction controls for the false rejection when all null hypotheses are true.
- However, in reality, only some of the $M$ null hypotheses may be true.
- Indeed, some p-values may be smaller than others.
- How about rejecting the null hypothesis with the smallest p-value first?
- Then, consider multiple testing with a smaller number of hypotheses.
...
- Equivalently, we can adjust the p values as
$$
\tilde{p}^{(m)} = \max_{l \le m} \min\{ \{ (M - l + 1)p^{(l)} \}, 1\},
$$
and reject $H_0^{(m)}$ if $\tilde{p}^{(m)} \le \alpha$.
...
- Neither Bonferroni correction nor Bonferroni-Holm correction exploits the correlation among test statistics.
...
- If the test statistics are _independent_, then the Benjamini-Hochberg correction controls for the FDR.
- If the test statistics are _dependent_, then the Benjamini-Hochberg correction controls for the FDR under the _positive regression dependence_ assumption.
- __Definition__: _Increasing set__: A set $D$ is said to be increasing if $x \in D$ and $y \ge x$ implies $y \in D$.
- __Definition__: _Positive regression dependence_: For test statistics $X^m$, $m = 1, \cdots, M$, for any increasing set $D$, and for any $m \in \mathcal{M}_0 \subset \mathcal{M}$, $\mathbb{P}\{\{X^m\}_{m \in \mathcal{M}_0} \in D| X^m = x\}$ is non-decreasing in $x$.
- If the joint distribution of the test statistics satisfies positive regression dependence for the set of true null hypothesis $\mathcal{M}_0 \subset \mathcal{M}$, then the Benjamini-Hochberg correction controls for the FDR no greater than $\alpha M_0/M$.


## General case

- In general, the Benjamini-Hochberg correction may not control for the FDR.
