...
- Abadie (2005).
- Let $p(X_i) = \mathbb{P}(G_i = 1| X_i)$ be the propensity score.
- We have:
$$
\begin{split}
&\mathbb{E}[Y_{i1} - Y_{i0}| G_i = 1, X_i] - \mathbb{E}[Y_{i1} - Y_{i0}| G_i = 0, X_i]\\
&= \frac{1}{p(X_i)} \mathbb{E}[G_i \cdot (Y_{i1} - Y_{i0})| X_i]\\
&\quad - \frac{1}{1 - p(X_i)} \mathbb{E}[(1 - G_i) \cdot (Y_{i1} - Y_{i0})| X_i]\\
&= \mathbb{E}\Bigg[\left(\frac{G_i}{p(X_i)} -  \frac{1 - G_i}{1 - p(X_i)} \right)\\
&\quad \cdot (Y_{i1} - Y_{i0}) \Bigg | X_i \Bigg].
\end{split}
$$
...
- The average treatment effect on treated under the conditional parallel trend assumption is:
$$
\begin{split}
\tau^{ATT}
&= \mathbb{E}[Y_{i1}(1) - Y_{i1}(0)|G_i = 1] \\
&= \int \mathbb{E}[Y_{i1}(1) - Y_{i1}(0)|G_i = 1, X_i] d\mathbb{P}(X_i|G_i = 1)\\
&= \int \{\mathbb{E}[Y_{i1} - Y_{i0}| G_i = 1, X_i]\\
&\quad - \mathbb{E}[Y_{i1} - Y_{i0}| G_i = 0, X_i]\} d\mathbb{P}(X_i|G_i = 1)\\
&= \int \mathbb{E}\Bigg[\left(\frac{G_i}{p(X_i)} -  \frac{1 - G_i}{1 - p(X_i)} \right)\\
&\quad \cdot (Y_{i1} - Y_{i0}) \Bigg | X_i \Bigg] d\mathbb{P}(X_i|G_i = 1).
\end{split}
$$

## Inverse probability weight (IPW) approach

- Since $d\mathbb{P}(X_i|G_i = 1) = \frac{p(X_i)}{\mathbb{P}(G_i = 1)} d\mathbb{P}(X_i)$:
$$
\begin{split}
\tau^{ATT}
&= \mathbb{E}\Bigg[\left(\frac{G_i}{p(X_i)} -  \frac{1 - G_i}{1 - p(X_i)} \right)\\
&\quad \cdot (Y_{i1} - Y_{i0}) \cdot \frac{p(X_i)}{\mathbb{P}(G_i = 1)} \Bigg]\\
&= \mathbb{E}\Bigg[\bigg(\frac{G_i}{\mathbb{P}(G_i = 1)}\\
&\quad - \frac{p(X_i)(1 - G_i)}{(1 - p(X_i))\mathbb{P}(G_i = 1)} \bigg)\\
&\quad \cdot (Y_{i1} - Y_{i0}) \Bigg].
\end{split}
$$
...
- Let $N_1 = \sum_{i = 1}^N G_i$.
- We normalize the control weights by the sample analogue of $\mathbb{P}(G_i = 1)$.
...
\left[
\frac{G_i}{N_1}
- \frac{\frac{\hat{p}(X_i)(1 - G_i)}{1 - \hat{p}(X_i)}}{
\sum_{j = 1}^N \frac{\hat{p}(X_j)(1 - G_j)}{1 - \hat{p}(X_j)}
}
\right]\\
&\quad \cdot (Y_{i1} - Y_{i0})
\end{aligned}
$$
...
- Sant'Anna and Zhao (2020) combines both approaches.
