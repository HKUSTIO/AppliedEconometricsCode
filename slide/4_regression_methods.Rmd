...
$$
- Remark that this linear regression function is correctly specified without loss of generality, because we can set:
$$
...
$$
- In other words, can we derive $\mathbb{E}(\epsilon_i | Z_i = z) = 0$ for $w = 0, 1$ from random sampling and random assignment?
...
$$
- We _do not_ assume that the conditional mean of $Y_i$ is actually linear in $Z_i$ and $W_i$.
