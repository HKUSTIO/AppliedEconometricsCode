---
title: "Clustered randomized experiments"
...
\end{pmatrix}^{-1},
$$
if $\forall g, \overline{Z}_g \in \{0, 1\}, \sum_{g = 1}^G \overline{Z}_g = G_t$ and $0$ otherwise.
...
where
$$
s_{Ct}^2 \equiv \frac{1}{G_t - 1} \sum_{g: \overline{Z}_g = 1}\left(\overline{Y}_g^{obs} - \frac{1}{G_t} \sum_{g': \overline{Z}_{g'} = 1} \overline{Y}_{g'}^{obs}\right)^2.
$$

## Estimate $\tau_C$ by a linear regression

- We obtain the same estimate by using the ordinary least squares method for a linear regression function:
$$
...
$$
we obtain an estimator identical to $\hat{\tau}_C$ if we run a weighted least squares method where unit $i$ is weighted by $1/N_{g(i)}$.


## Estimate $\tau_{fs}$ by a linear regression

- We can estimate $\tau_{fs}$ by running an (unweighted) ordinary least squares method for the unit-level linear regression function:
$$
Y_i^{obs} = \alpha + \tau \cdot Z_i + \epsilon_i.
$$
