...
$$
where the subscript $fs$ denotes the finite-sample parameter.
...
D_i = Z_i - \frac{N_1}{N}.
$$
- Because $\mathbb{E}(D_i) = 0$ under a completely randomized assignment, $\mathbb{E}(\hat{\tau}_{fs}) = \tau_{fs}$: unbiased.
...
$$
s_0^2 \equiv \frac{1}{N_0 - 1} \sum_{i: Z_i = 0} (Y_i - \overline{Y}_0)^2,
$$
$$
s_1^2 \equiv \frac{1}{N_1 - 1} \sum_{i: Z_i = 1} (Y_i - \overline{Y}_1)^2.
$$
...
- If this is the estimand, the sampling variance of $\hat{\tau}_{fs}$ due to the randomness of $\mathbf{W}, \mathbf{Y}^\ast(1)$, and $\mathbf{Y}^\ast(0)$ is, after lengthy algebra:
$$
